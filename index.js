(function() {
  ...
  var _sfc_render$d = function render() {
    ...
    return _vm.ready ? _c("ul", { staticClass: "lbvs-changes" }, _vm._l(_vm.changes, function(status, path) {
      return _c("li", { key: path }, [_c("div", [_c("k-input", { attrs: { "id": path, "name": path, "value": status, "checked": typeof _vm.markedChangesForStaging[path] !== "undefined", "type": "checkbox" }, on: { "input": ($event) => _vm.updateChange(path, status, $event) } })], 1), _c("span", { attrs: { "data-status": status, "title": _vm.$t("versions.label.status." + status) } }, [_vm._v(" " + _vm._s(status) + " ")]), _c("label", { attrs: { "for": path } }, [_vm._v(_vm._s(path))])]);
    }), 0) : _vm._e();
  };
  ...
  const _sfc_main$4 = {
    ...
    computed: {
      canCreateVersion() {
        return this.$permissions["lukasbestle.versions"].create === true && Object.keys(this.markedChanges).length > 0;
      },
      canCheck() {
        return this.$permissions["lukasbestle.versions"].create === true && Object.keys(this.markedChanges).length > 0;
      },
  ...
  var _sfc_render$3 = function render() {
    ...
    return _c("div", { staticClass: "lbvs-version" }, [_c("strong", [_vm._v(_vm._s(_vm.version.label))]), _c("dl", { staticClass: "lbvs-version-details" }, [_vm._l(_vm.mergedDetails, function(detail) {
      return [_c("dt", { key: detail.title, staticClass: "sr-only" }, [_vm._v(_vm._s(detail.title) + ":")]), _c("dd", { key: detail.title, attrs: { "title": detail.title } }, [_vm._v(" " + _vm._s(detail.value) + " ")])];
    })], 2)]);
  };
